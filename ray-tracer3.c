 ...
 */
float magnitude_f(float x, float y, float z){
 ...
 * out: Color3
 */
Color3 evaluateFiniteScatteringDensity(bsdf_ptr bsdf, Vector3 w_i, Vector3 w_o, Vector3 n){
 ...
 */
float intersect(ray_ptr R, triangle_ptr T, float *weight){
  ...
  int bool1 = (a <= epsilon) || (weight[0] < (-1)*epsilon2) || weight[1] < (-1)*epsilon2 || weight[2] < (-1)*epsilon2 || dist <= 0.0f;

  /*  if ((a <= epsilon) || (weight[0] < (-1)*epsilon2) ||
     (weight[1] < (-1) * epsilon2) || (weight[2] < (-1)*epsilon2) ||
     (dist <= 0.0f)) {
  */
  if(bool1){
...
/* visible() always returns true (temporarily)
 */
int visible(Point3 P, Vector3 direction, float distance, scene_ptr scene) {
  float rayBumpEpsilon = 1e-4;
...
/* rayTrace: trace one ray for every pixel in a given scene, given eye viewpoint and range of pixels
 * Args: Image image, Scene scene, Eye eye, x0/x1/y0/y1 as start and end pixels
 */
void rayTrace(image_ptr image, scene_ptr scene, eye_ptr eye, int x0, int y0, int x1, int y1){
...
// Helper function to make one triangle scene
void makeTriangleScene(scene_ptr s, char **argv){
  ...
  for(int k = 0; k < s->triangleCount; k++){
    ...
    for(int i = 0; i < 9; i++){
      ...
      if(i%3 == 2){
	s->triangleArray[k].vertex[i/3][i%3] += -2;
      }
    }
...
image_ptr render(char **argv){
  ...
  rayTrace(image, testScene, eye, 0, 0, image->width, image->height);

  free(testScene->triangleArray[0].bsdf);
...
int main(int argc, char **argv){
  ...
  write_JPEG_file(argv[3], to, OUT_QUALITY);
  destroy_frame(to);
